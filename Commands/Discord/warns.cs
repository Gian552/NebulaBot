...
{
    public class Warns : InteractionModuleBase<SocketInteractionContext>
    {
        [SlashCommand("warns", "Zeigt Alle Warns eins Spielers an.")]
        public async Task Execute(string steamID)
        {
            ...
            if (_userStats.Warns.Count == 0)
            {
                await RespondAsync("Dieser Spieler hat keine Warns.");
                return;
            }
            ...
            foreach (var warn in _userStats.Warns)
            {
